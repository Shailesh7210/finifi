...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    allowed.includes(file.mimetype) ? cb(null, true) : cb(new Error('Only PDF, JPEG, PNG supported'));
  },
});
...
router.post('/upload', upload.single('file'), async (req, res, next) => {
  try {
    ...
    try {
      ...
      await Document.findByIdAndUpdate(doc._id, { parseStatus: 'failed', parseError: parseErr.message });
      return res.status(422).json({ documentId: doc._id, error: 'Parsing failed', detail: parseErr.message });
    }
    ...
    if (poNumber) {
      try {
        ...
        console.error(`Match trigger failed for PO ${poNumber}:`, matchErr.message);
      }
    }
...
// GET /documents?poNumber=&documentType=
